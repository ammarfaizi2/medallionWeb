$(function() {
		...
		if ('${fromCancel}' == 'fromCancel'){
			...
			if ((("${flash.error}" || "${errors}") && ('${mode}'=='entry')) || 
				(("${flash.error}" || "${errors}") && ('${mode}'=='edit'))){
		...
		if ('${fromCancel}' == 'fromCancel'){
			if ( jQuery.trim($('#statusHidden').val()) == 'E' || jQuery.trim($('#statusHidden').val()) == 'WS') {
		...
		function attachSecuritiesPaging() {
			...
			$('#securityCode').dynapopup2({key:'securityKey', help:'securityCodeHelp', desc:'securityCodeDesc'}, pickName, securityType, 'actionCode', function(data){
				if (data) {
					$('#securityCode').removeClass('fieldError');
			...
			get:{
				...
				success: function(data) {
					if (data) {
						...
						$('#hasExercisePriceOri').val(data.hasExercisePrice);
...
//						$('#interestRate').val(interestRate);
		...
		if (('${mode}'=='entry')&&('${confirming}'!='true')){
			if ($('#taxObject').val()==''){
				...
				$('p[id=pExercisePrice] label span').html("");
				$('p[id=pActionTemplateLink] label span').html("");
				if ($('#targetType').val() == '${targetTypeOther}'){
					$('#securityTypeTarget').removeAttr('disabled');
					$('#securityTypeTargetHelp').removeAttr('disabled');
					$('#securityCodeTarget').removeAttr('disabled');
					$('#securityCodeTargetHelp').removeAttr('disabled');
		...
		$('#taxable1').add($('#taxable2')).click(function(){
			...
			if ($('#taxable1').is(':checked')){
				...
				if ($('#hasExercisePriceOri').val()=='true'){
					$('#hasExercisePrice').val('true');
					$('p[id=pExercisePrice] label span').html(" *");
					$('#exercisePrice').removeAttr("disabled");
				} else {
					$('#hasExercisePrice').val('false');
					$('#exercisePrice').val("");
					$('#exercisePriceStripped').val("");
					$('p[id=pExercisePrice] label span').html("");
					$('#exercisePrice').attr("disabled","disabled");
		...
		$("#announcementDate").change(function(){
			...
			var errorMsg = "must be greater or equal than Announcement Date";
...
//			if (($(this).val()!='')&&(!$(this).hasClass('fieldError'))&&($(idDate2).val()!='')&&(!$(idDate2).hasClass('fieldError'))){
		...
		$("#cumDate").change(function() {
			...
			var errorMsg1 = "must be greater or equal than Announcement Date";
			var errorMsg2 = "must be greater or equal than Cum Date";
...
//				if (($(idDate3).val()!='')&&(!$(idDate3).hasClass('fieldError'))){
		...
		$("#exDate").change(function() {
			...
			var errorId1 = idDate2+"Error";
			var errorId2 = idDate3+"Error";
			var errorMsg1 = "must be greater or equal than Cum Date";
			var errorMsg2 = "must be greater or equal than Ex Date";
...
//				if (($(idDate3).val()!='')&&(!$(idDate3).hasClass('fieldError'))){
		...
		$("#recordingDate").change(function() {
			...
			var errorId1 = idDate2+"Error";
			var errorId2 = idDate3+"Error";
			var errorMsg1 = "must be greater or equal than Ex Date";
			var errorMsg2 = "must be greater or equal than Recording Date";
...
//				if (($(idDate3).val()!='')&&(!$(idDate3).hasClass('fieldError'))){
		...
		$("#distributionDate").change(function() {
			...
			var date2 = new Date($(idDate2).datepicker('getDate')).getTime();
			var errorId = idDate2+"Error";
			var errorMsg = "must be greater or equal than Recording Date";
