 ...
 * ========================================================================= */
		...
		function constactFilters() {
			...
			try{
				var result = html.fetch("@{RecondProfile.fetchFilter()}", {"profileKey" : profileKey});
...
//			app.generate.button("option", "disabled", condition);
		...
		function constractTable(data) {
			...
			app.saveToCsv.attr("href", "/recondprofile/processGenerateErrorFile?param.batchNo="+data["BATCHID"]+"&param.output=csv");
			...
			table = app.tablePopulate.paging("@{RecondProfile.previewPagingDownload()}", parent, function(p){
		...
		var generateFile = function(successOnly){
			...
			var result = html.fetch("@{RecondProfile.processGenerateFile()}", {"param" : param, "uniqe":app.gDate.val(), "successOnly":successOnly}, function(data){
		...
		app.populate.click(function(){
			...
			if (validate()){
				...
				try{
					...
					var result = html.fetch("@{RecondProfile.processRunDownload()}", {"profileId" : app.updProfileKey.val(), "filters":filters, "uniqe":app.gDate.val()});
					if (result["ERROR"]) {
