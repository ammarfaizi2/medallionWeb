 ...
 * ========================================================================= */
		...
		var wek = this;
		app.datatable = app.tblTransactionSubscribeRedeem.paging("@{RegistryCancelTrade.pagingCancelTradeRedeem()}", wek);
		...
		function changeTableTransaction() {
			...
			if(app.type.val() == TYPE_SWITCHING)
			{
				if(app.datatable.attr("id") != app.tblTransactionSwitching.attr("id"))
				{
					...
					app.datatable = app.tblTransactionSwitching.paging("@{RegistryCancelTrade.pagingCancelTradeSwitching()}", wek);
				}
			...
			if(app.type.val() == TYPE_DIVIDEND)
			{
				if(app.datatable.attr("id") != app.tblTransactionDividend.attr("id"))
				{
					...
					app.datatable = app.tblTransactionDividend.paging("@{RegistryCancelTrade.pagingCancelTradeDividend()}", wek);
				}
...
//			if (($('#tradeDateFrom').val()!='') && ($('#transactionDateTo').val()!='')) { 
...
//				location.href = '@{RegistryCancelTrade.cancelSubscription()}?keyId=' + keySubscribe + '&from=cancelTrade';
				location.href = '@{RegistrySubscription.cancel()}?keyId=' + keySubscribe + '&from=cancelTrade';
			}
...
//				location.href = '@{RegistryCancelTrade.cancelRedemption()}?keyId=' + keyRedeem + '&from=cancelTrade' + '&liquidation=' + liquidationRedeem;
				location.href = '@{RegistryRedemption.cancel()}?keyId=' + keyRedeem + '&from=cancelTrade' + '&liquidation=' + liquidationRedeem;
			}
		});
...
//				location.href = '@{RegistryCancelTrade.cancelSwitching()}?keyId=' + keySwitching + '&from=cancelSwitching';
				location.href = '@{RegistrySwitch.cancel()}?keyId=' + keySwitching + '&from=cancelSwitching';
			}
		});
...
//				location.href = '@{RegistryCancelTrade.cancelDividend()}?keyId=' + keyDividend + '&from=cancelFundAction';
				location.href = '@{RegistryFundAction.cancel()}?keyId=' + keyDividend + '&from=cancelFundAction';
			}
		});
