...
@With(Secure.class)
public class BookTransfers extends MedallionController {
	private static Logger log = Logger.getLogger(BookTransfers.class);
	...
	public static void back(String messageId, String mode, String status) {
		...
		CsAccount bankAccount = accountService.getAccountByNo(UIConstants.SIMIAN_BANK_ID, cBestMessage.getData().getAccountnumber());
		...
		renderArgs.put("orderingaccountname", bankAccount.getName());
		...
		flash.put(UIConstants.BREADCRUMB, applicationService.getMenuBreadcrumb(MenuConstants.CB_BOOK_TRANSFER));
		render("BookTransfers/detail.html", cBestMessage, mode, status);
	}
	...
	public static void entry() {
		...
		flash.put(UIConstants.BREADCRUMB, applicationService.getMenuBreadcrumb(MenuConstants.CB_BOOK_TRANSFER));
		render("BookTransfers/detail.html", cBestMessage, mode);
	}
	...
	public static void save(CBestMessage cBestMessage, String mode, String status) {
		...
		if (cBestMessage != null) {
			...
			validation.required("Instruction Type", cBestMessage.getData().getInstructiontype());
			validation.required("Participant Code", cBestMessage.getData().getParticipantcode());
			validation.required("Ordering Account", cBestMessage.getData().getAccountnumber());
			...
			if (cBestMessage.getData().getAccountnumber().equalsIgnoreCase(cBestMessage.getData().getBeneficiaryaccount())) {
				validation.addError("", "Ordering Account should not equal with Beneficiary Account", "Beneficiary Account");
			}
			...
			if (validation.hasErrors()) {
				...
				if ((cBestMessage.getData().getAccountnumber() != null) && (cBestMessage.getData().getAccountnumber().trim().length() > 0)) {
					CsAccount bankAccount = accountService.getAccountByNo(UIConstants.SIMIAN_BANK_ID, cBestMessage.getData().getAccountnumber());
					renderArgs.put("orderingaccountname", bankAccount.getName());
				}
	...
	public static void confirm(String mode, CBestMessage cBestMessage, String status) {
		...
		cBestMessage.setName("BookTransfer");
		cBestMessage.setType("OutgoingMessage");

		cbestService.saveCBestMessage(MenuConstants.CB_BOOK_TRANSFER, cBestMessage, session.get(UIConstants.SESSION_USERNAME), "", session.get(UIConstants.SESSION_USER_KEY));

		render("BookTransfers/detail.html", new CBestMessage(), mode);
	}
	...
	public static void approval(String taskId, Long keyId, String from) {
		...
		if (from.equals(UIConstants.WORKFLOW_FROM)) {
			...
			render("BookTransfers/approval.html", cBestMessage, taskId, mode);
		} else {
